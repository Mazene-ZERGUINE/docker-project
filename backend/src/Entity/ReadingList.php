...
class ReadingList
{
    ...
    #[ORM\OneToMany(mappedBy: 'readingList', targetEntity: user::class)]
    private Collection $user_id;

    #[ORM\OneToMany(mappedBy: 'readingList', targetEntity: books::class)]
    ...
    public function removeUserId(user $userId): self
    {
        if ($this->user_id->removeElement($userId)) {
            ...
            if ($userId->getReadingList() === $this) {
                $userId->setReadingList(null);
            }
        }
    ...
    public function removeIsbn(books $isbn): self
    {
        if ($this->isbn->removeElement($isbn)) {
            // set the owning side to null (unless already changed)
            if ($isbn->getReadingList() === $this) {
                $isbn->setReadingList(null);
            }
        }
